...
public final class BruteForceHammingMatcher implements StrMatchInterface {
    ...
    @Override
    public @NotNull IntList applyUnchecked(byte @NotNull [] haystack, byte @NotNull [] needle, int start, int end) {
        var haystackPos = start;
        final var retl = new IntArrayList();
        while (haystackPos + needle.length < end) {
            if (StrDistance.hammingDistance(haystack, needle, haystackPos, haystackPos + needle.length, 0, needle.length) <= maxDistance) {
